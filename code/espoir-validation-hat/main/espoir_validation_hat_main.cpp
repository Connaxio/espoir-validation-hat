...
uint8_t digital_io_pins[] = { GPIO_NUM_2, GPIO_NUM_4, GPIO_NUM_5, GPIO_NUM_9, GPIO_NUM_10, GPIO_NUM_12,
...
uint8_t adc1_pins[num_channels] = { 34, 36, 37, 38, 39 };
uint8_t adc2_pins[num_channels] = { 4, 12, 13, 14, 15 };
...
void validate_board(void *pvParameters) {
	...
	if (err == ESP_OK) {
		uint32_t adc1_readings_150[num_channels] = { 0 };
		uint32_t adc2_readings_150[num_channels] = { 0 };
		uint32_t adc1_readings_850[num_channels] = { 0 };
		uint32_t adc2_readings_850[num_channels] = { 0 };
		...
		if (err == ESP_OK) {
			...
			for (uint8_t j = 0; j < (1 << avg_pow); ++j) {
				static_cast<ADS1119*>(adc)->updateChannel(0);
				adc_ref_readings[0] += adc->getValue("ch0");
			}
...
//            printf("ADC2 readings:\t%i\t%i\t%i\t%i\t%i\r\n", adc2_readings_150[0], adc2_readings_150[1],
//                    adc2_readings_150[2], adc2_readings_150[3], adc2_readings_150[4]);
//            printf("Ref ADC voltage:\t%.3f V\r\n", adc_ref_readings[0]);
		...
		if (err == ESP_OK) {
			...
			for (uint8_t j = 0; j < (1 << avg_pow); ++j) {
				static_cast<ADS1119*>(adc)->updateChannel(0);
				adc_ref_readings[1] += adc->getValue("ch0");
			}
...
//                    adc2_readings_850[2], adc2_readings_850[3], adc2_readings_850[4]);
//            printf("Ref ADC voltage:\t%.3f V\r\n", adc_ref_readings[1]);
		...
		if (err == ESP_OK) {
			...
			A1_stdev = sqrt(1.0f * A1_stdev / num_channels);
			B1_stdev = sqrt(1.0f * B1_stdev / num_channels);
			A2_stdev = sqrt(1.0f * A2_stdev / num_channels);
			B2_stdev = sqrt(1.0f * B2_stdev / num_channels);
	...
	if (err == ESP_OK) {
		...
		for (uint8_t i = 0; i < digital_io_count; ++i) {
			...
			if (err == ESP_OK) {
				...
				vTaskDelay(settling_ticks);
				err = static_cast<ADS1119*>(adc)->updateChannel(0);
				value = adc->getValue("ch0");
	...
	if (err == ESP_OK) {
		...
		float error_12V = (adc->getValue("ch2") * 11.2 - 12000) / 12000;
		float error_5V6 = (adc->getValue("ch1") * 6.1 - 5600) / 5600;
		float error_3V3 = (adc->getValue("ch3") * 6.1 - 3300) / 3300;

		if (abs(error_12V) > 0.05f || abs(error_5V6) > 0.05f || abs(error_3V3) > 0.01f) {
			err = ESP_ERR_INVALID_STATE;
		}

		printf("12V error:\t%.2f%%\r\n", error_12V * 100.0f);
		printf("5.6V error:\t%.2f%%\r\n", error_5V6 * 100.0f);
